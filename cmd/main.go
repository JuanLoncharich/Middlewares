...
import (
	...
	"github.com/security-eval/mcp-eval-operator/internal/controller"
)
...
func main() {
	...
	// Disable HTTP/2 by default to mitigate HTTP/2 stream-cancellation and
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:       "a7c4e2f1.security.eval.io",
	})
	if err != nil {
	...
	images := controller.ImageConfigFromEnv()
	setupLog.Info("evaluation job image configuration",
		"cloner", images.ClonerImage,
		"target", images.TargetImage,
		"evaluator", images.EvaluatorImage,
	...
	if err = (&controller.MCPEvaluationRunReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("mcpevaluationrun-controller"),
		Images:   images,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "MCPEvaluationRun")
		os.Exit(1)
	}
	...
	// Readiness must reflect actual API-server reachability, not just process
	// liveness: a manager that cannot reach the API server must not receive
	// traffic (or, with leader election, keep its lease) while wedged.
	if err := mgr.AddReadyzCheck("apiserver", controller.NewAPIServerReadinessCheck(cfg)); err != nil {
